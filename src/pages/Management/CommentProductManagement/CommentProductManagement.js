...
const CommentProductManagement = () => {
  ...
  async function handleOk1(data) {
    try {
      if (current) {
        await axiosRes().put(`${API_COMMENT_PRODUCT_URL}/${current.id}`, data);
  ...
  async function handleOk2(data) {
    try {
      if (currentRep) {
        ...
        dispatch(updateRepCommentProduct({ ...current, ...data }));
        dispatch(
          showToast({
            isOpen: true,
            text: "Sửa thành công",
        ...
        const res = await axiosRes().post(`${API_REPLIED_COMMENT_URL}`, data);
