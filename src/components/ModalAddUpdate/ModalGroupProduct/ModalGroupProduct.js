...
const ModalGroupProduct = (props) => {
  ...
  const [indexCategory, setIndexCategory] = useState(
    categories.findIndex((item) => current && current.categoryId === item.id)
  );
      ...
      handleOk={() => {
        if (indexCategory !== -1)
          handleOk({
            ...
            categoryId: categories[indexCategory].id,
            isHot,
          });
      }}
    >
        ...
        <Grid container spacing={2}>
          <Grid item xs={6}>
            <div className="form-group">
              <label htmlFor="group-product">Chọn danh mục</label>
              <select
                id="group-product"
                value={indexCategory}
...
export default ModalGroupProduct;
